...
def chat_query(config_values, conn, user_values={}, prompt="", conversation=[]):
    ...
    #the following call adds to document set the missing documents in a sequence. the goal is to add missing info into aborted semantic
    extended_docs_list=fix_aborted_sequences(extended_docs_list,3) #max missing chunks 
    logging.debug(f"With fixed aborted sequences: {extended_docs_list}")
    ...
    if docs_ids != "()":
        ...
        # Let's optimize the retrieved info. In case of adjacent documents we need to join them into single doc removing overlapping part
        # since originally documents were generated by overlapping 
        #create list of lists, each sublist is a documents sequence 
        docs_optimized=group_sequences(docs)

        assist_info="\nPlease use the following data set as information for the query:\n"
        for doc in docs_optimized:    
